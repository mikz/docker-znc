# ZNC for Docker [![Docker Repository on Quay.io](https://quay.io/repository/mikz/znc/status "Docker Repository on Quay.io")](https://quay.io/repository/mikz/znc)
...
from the host to `/znc-data` in the container. For example:
    ...
    docker run -d -p 6667 -v $HOME/.znc:/znc-data quay.io/mikz/znc
...
Or if you want to specify which port to map the default 6667 port to:
    ...
    docker run -d -p 36667:6667 -v $HOME/.znc:/znc-data quay.io/mikz/znc
...
[start-znc]: https://github.com/mikz/docker-znc/blob/master/start-znc

For example, if you want to use the `--makepass` option, you would run:

    docker run -i -t -v $HOME/.znc:/znc-data quay.io/mikz/znc --makepass
...
2. Checkout source: `git clone https://github.com/mikz/docker-znc.git && cd docker-znc`
